...
!          William H. Lipscomb, LANL
      ...
      use ice_fileunits, only: nu_forcing
      use ice_forcing, only: ocn_data_dir, atm_data_dir
      ! afm 20190506 - end 
      ...
      real (kind=dbl_kind), dimension(nx_block,ny_block,max_blocks) :: &
         ainit_arr, hinit_arr    ! initial area, thickness
      integer (kind=int_kind) :: &
         nbits
      character (char_len_long) :: &        ! input data file names
         sic_file,  sih_file
      logical (kind=log_kind) :: dbugtmp
      ! afm 20190506 - end 
      ...
      !! afm 20190506 -- begin
      ! read ice initial conc & thickness

!      sic_file  = trim(ocn_data_dir)//'sic_gofs_regional_20190210.dat'
!      sih_file  = trim(ocn_data_dir)//'sih_gofs_regional_20190210.dat'
!       sic_file  = trim(ocn_data_dir)//'sic_gofs_regional_20181210.dat'
!       sih_file  = trim(ocn_data_dir)//'sih_gofs_regional_20181210.dat'
!       sic_file  = trim(atm_data_dir)//'sic_cfsv2_20171001.dat'
!       sih_file  = trim(atm_data_dir)//'sih_cfsv2_20171001.dat'
       sic_file  = trim(atm_data_dir)//'sic_cfsv2_ini.dat'
       sih_file  = trim(atm_data_dir)//'sih_cfsv2_ini.dat'

!       sic_file  = trim(atm_data_dir)//'sic_gfs_ini.dat'   !hhu
!       sih_file  = trim(atm_data_dir)//'sih_gfs_ini.dat'   !hhu
!       sic_file  = trim(ocn_data_dir)//'sic_hycom_ini.dat'   !hhu
!       sih_file  = trim(ocn_data_dir)//'sih_hycom_ini.dat'   !hhu

      nbits = 64
      dbugtmp = .true.

      if (my_task == master_task) &
              call ice_open (nu_forcing, sic_file, nbits)

      call ice_read(nu_forcing, 1, ainit_arr, 'rda8', dbugtmp, &
                           field_loc_center, field_type_scalar)

      if (my_task == master_task) &
              call ice_open (nu_forcing, sih_file, nbits)
      call ice_read(nu_forcing, 1, hinit_arr, 'rda8', dbugtmp, &
                           field_loc_center, field_type_scalar)
...
!          William H. Lipscomb, LANL
      ...
      real (kind=dbl_kind), dimension (nx_block,ny_block,ncat) :: &
         ainit_arr_cat
      real (kind=dbl_kind), dimension (nx_block,ny_block) :: sum_arr
      ! afm 20190507 -- end
...
! afm 20190507 -- begin
        elseif (trim(atm_data_type) == 'wak') then
         ...
         do n = 1, ncat
            ...
            ainit_arr_cat(:,:,n) = &
        max(c0, (c2*hinit_arr_b(:,:)*hinit(n) - hinit(n)**2))
            sum_arr = sum_arr + ainit_arr_cat(:,:,n)
         enddo
         do n = 1, ncat
            ainit_arr_cat(:,:,n) = ainit_arr_cat(:,:,n) / &
        ( sum_arr + puny/ncat ) * ainit_arr_b(:,:) ! normalize
         enddo
! afm 20190507 -- end 
         else
      ...
      !   hbar = 1.5_dbl_kind  ! initial ice thickness with greatest area
         hbar = 0.5_dbl_kind  ! initial ice thickness with greatest area
         ...
         do i = ilo, ihi
            if (tmask(i,j)) then
               if ( ainit_arr_b(i,j) > 0.0_dbl_kind .and. &
                    hinit_arr_b(i,j) > 0.0_dbl_kind ) then
                 icells = icells + 1
                 indxi(icells) = i
                 indxj(icells) = j
!print*, "ice initW",i,j,ulon(i,j)*rad_to_deg,tlat(i,j)*rad_to_deg   
            endif            ! ULON
            endif               ! tmask
         enddo                  ! i
         enddo                  ! j
...
!               if ( TLAT(i,j) > 70.0/rad_to_deg ) then 
!               if ( sst (i,j) <= Tf(i,j)+0.1_dbl_kind ) then
               if ( sst (i,j) <= Tf(i,j)+0.01_dbl_kind ) then
                  icells = icells + 1
                  indxi(icells) = i
                  indxj(icells) = j
!print*, i,j,"place ice at high latitudes where ocean sfc is cold"
               endif            ! cold surface
            endif               ! tmask
         enddo                  ! i
         enddo                  ! j
...
!                                         / (real(ny_global,kind=dbl_kind)) * p5)
                ...
                ! afm 20190507 -- begin
                elseif (trim(atm_data_type) == 'wak') then
                  aicen(i,j,n) = ainit_arr_cat(i,j,n)
                  vicen(i,j,n) = hinit(n) * ainit_arr_cat(i,j,n)! m
                ! afm 20190507 -- end 
